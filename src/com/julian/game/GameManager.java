...
public class GameManager extends AbstractGame{
	...
	public void moveEntity(Entity e, float dt) {
		...
		// Example Code: Move until wall, then "bounce" back
		if (e.lastDx == 0 && e.lastDy == 0) {
		...
		// Keep moving and test for walls
		float dx = e.lastDx;
		float dy = e.lastDy;
		if (e.xPosition + e.lastDx < 0 || e.xPosition + e.lastDx > width) {
			dx = -e.lastDx;
		}
		if (e.yPosition + e.lastDy < 0 || e.yPosition + e.lastDy > hight) {
			dy = -e.lastDy;
		}
		e.updatePosition(dx, dy, dt, width, hight);
